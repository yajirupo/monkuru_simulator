...
## VS リプレイファイルを読み込み、プレイヤー情報と入力列を復元する。
...
## VS 対戦終了時に保存済みリプレイ情報をファイルへ書き出す。
func vs_replay_data_write(num: int) -> void:
	if not GameState.has_last_vs_replay or GameState.last_vs_replay_is_com:
		return
...
## 記録済み入力を復元し、VS 再生用の use_key に反映する。
